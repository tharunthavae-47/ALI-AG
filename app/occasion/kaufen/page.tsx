...
export default function KaufenPage() {
  ...
  useEffect(() => {
    if (!chatId) return
    const channel = supabase.channel(`occasion-customer-chat-${chatId}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "occasion_messages", filter: `chat_id=eq.${chatId}` }, (payload) => setMessages((prev) => prev.some((m) => m.id === payload.new.id) ? prev : [...prev, payload.new])).subscribe()
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 pt-16">
        ...
        {listings.length === 0 ? <div className="mt-12 rounded-3xl border border-white/10 bg-zinc-950 p-12 text-center text-zinc-500">Aktuell sind keine Fahrzeuge online.</div> : <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-3">{listings.map((listing) => { const first = images.find((i) => i.occasion_request_id === listing.occasion_request_id); return <button key={listing.id} onClick={() => { setSelected(listing); setChatId(null) }} className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-950 text-left transition hover:-translate-y-1 hover:border-white/25">{first ? <img src={imageUrl(first.image_url)} alt={`${listing.marke} ${listing.modell}`} className="aspect-[16/10] w-full object-cover" /> : <div className="flex aspect-[16/10] items-center justify-center text-5xl">🚗</div>}<div className="p-6"><p className="text-xs uppercase tracking-widest text-zinc-500">{listing.jahrgang}</p><h2 className="mt-1 text-2xl font-bold">{listing.marke} {listing.modell}</h2><p className="mt-3 text-sm text-zinc-400">{Number(listing.kilometer).toLocaleString("de-CH")} km · {listing.treibstoff} · {listing.getriebe}</p><p className="mt-5 text-xl font-semibold">CHF {Number(listing.preisvorstellung).toLocaleString("de-CH")}</p></div></button> })}</div>}
      </section>

      {selected && <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 p-4 backdrop-blur-sm"><div className="mx-auto max-w-5xl rounded-3xl border border-white/10 bg-zinc-950 p-6 md:p-8"><div className="flex items-center justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-zinc-500">Fahrzeug</p><h2 className="mt-1 text-3xl font-bold">{selected.marke} {selected.modell}</h2></div><button onClick={() => setSelected(null)} className="rounded-full border border-white/10 px-4 py-2 text-sm">Schliessen</button></div>
