...
export async function sendBookingEmail(type: BookingEmailType, data: BookingEmailData) {
  ...
  if (!cfg) {
    console.error("[MB-Performance] Resend ist nicht konfiguriert: RESEND_API_KEY oder RESEND_FROM_EMAIL fehlt.")
  ...
  if (type === "new-owner") {
    ...
    if (!owner) {
      console.error("[MB-Performance] BOOKING_OWNER_EMAIL fehlt oder ist leer.")
      return { ok: false, error: "BOOKING_OWNER_EMAIL fehlt oder ist leer." }
    }

    if (!isValidEmail(owner)) {
      console.error("[MB-Performance] BOOKING_OWNER_EMAIL ist ungültig.")
    ...
    if (!customerEmail || !isValidEmail(customerEmail)) {
      console.error("[MB-Performance] Kunden-E-Mail ist ungültig.")
    ...
    subject =
      type === "confirmed"
        ...
        : type === "rejected"
          ? "Ihre Terminanfrage bei MB-Performance wurde abgelehnt"
          : "Ihre Terminanfrage bei MB-Performance ist eingegangen"
    html = layout(subject, subject, customerBody(data, type))
  }

  console.log(`[MB-Performance] Sende ${type} E-Mail an ${to}`)
  ...
  if (result.error) {
    console.error(`[MB-Performance] Resend Fehler bei ${type}:`, result.error)
    return { ok: false, error: result.error.message }
  }

  console.log(`[MB-Performance] ${type} E-Mail erfolgreich an Resend übergeben. ID: ${result.data?.id ?? "unbekannt"}`)
