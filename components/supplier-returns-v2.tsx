...
type Credit = { id: string; amount: number; remaining_amount: number; status: string; created_at: string; note: string | null }
...
export function SupplierReturnsV2() {
  ...
  async function load() {
    ...
    if (mode === "supplier") {
      const { data: cd } = await supabase.from("supplier_credits").select("id,amount,remaining_amount,status,created_at,note").eq("supplier_id", user.id).eq("status", "offen").order("created_at", { ascending: false })
      setCredits((cd as Credit[]) || [])
    } else {
      setCredits([])
    }
  }

  const creditBalance = useMemo(() => credits.reduce((sum, c) => sum + Number(c.remaining_amount || 0), 0), [credits])
  ...
  async function reviewReturn(entry: ReturnRequest, status: "bestaetigt" | "abgelehnt") {
    ...
    if (error) {
      ...
      setOwnerNote("")
      setMessage(`Retoure bestätigt. ${money(Number(result.return_value || 0))} wurden als Gutschrift hinterlegt. Neuer Auftragswert: ${money(Number(result.new_total || 0))}.`)
      await load()
    }
    setSaving(false)
  }
    ...
    return <section className="mx-auto mt-14 max-w-6xl border-t border-border px-4 pt-10 sm:px-6">
      <div className="border border-border bg-card p-5 sm:p-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="font-display text-xs uppercase tracking-[0.3em] text-muted-foreground">Retouren & Guthaben</p><h2 className="mt-2 font-display text-2xl font-bold uppercase tracking-wide">Teile zurückgeben</h2></div>
          <div className="border border-border bg-background px-5 py-4"><p className="text-xs uppercase tracking-widest text-muted-foreground">Aktuelles Guthaben</p><p className="mt-1 text-2xl font-bold">{money(creditBalance)}</p><p className="mt-1 text-xs text-muted-foreground">Wird bei zukünftigen Rechnungen verrechnet</p></div>
        </div>
        ...
        {selectedValue > 0 && <div className="mt-5 border border-border p-4 text-sm"><div className="flex justify-between"><span>Retoure</span><b>- {money(selectedValue)}</b></div><div className="mt-2 flex justify-between"><span>Neuer Auftragswert</span><b>{money(Math.max(0, Number(order?.adjusted_total_amount || 0) - selectedValue))}</b></div>{Number(order?.paid_amount || 0) > Math.max(0, Number(order?.adjusted_total_amount || 0) - selectedValue) && <div className="mt-2 flex justify-between"><span>Gutschrift</span><b>{money(Number(order?.paid_amount || 0) - Math.max(0, Number(order?.adjusted_total_amount || 0) - selectedValue))}</b></div>}</div>}
